...
HOSTNAME=$(echo "$URL" | sed -E 's_.*://([^/]+).*_\1_')

echo "Verifying DNS for host: $HOSTNAME..."

# Use dig to resolve the hostname to an IP. +short provides clean output.
# The -z flag checks if the output string is empty.
if [ -z "$(dig +short "$HOSTNAME")" ]; then
  echo "Error: DNS resolution failed for $HOSTNAME. Unable to find an IP address. ❌"
  exit 1
else
...
do
  ...
  TIME_TOTAL=$(echo "$CURL_OUTPUT" | cut -d'|' -f3)
