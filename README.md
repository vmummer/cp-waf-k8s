# cp-waf-k8s
Check Point WAF deployment on kubernetes and Microk8s

This is a conintuance of building on a previous deployment of WAF embedded nano-agent in a docker compose environment. This environment is deploying the Check Point WAF in a Kubernetes Cluster using microk8s in this testing development.

The environment includes the following K8S pods:

- Ingress Controller - Allow access into the K8S node
- Check Point WAF - Web and API monitoring and protection
- Test Host - Test host to create good and malicous traffic. Also to demonstrate the API auto learning and protection.
- Juice Shop Host - Juice Shop is a website used to teach and demonstrate web sight Vulnerablity. https://owasp.org/www-project-juice-shop
- Vampi API Host - Vampi is a website used to teach and demonstrate API hosted Vulnerablity. - https://github.com/erev0s/VAmPI



>>
The enhancement allows for running on Windows WSL if required or any current standard linux distribution. It also includes an additional Client Host for traffic generations of good and bad web and API traffic.

The deployment includes four containers, Check Point combined WAF and Nginx container(combined), tester host with traffic generators, OWASP JuiceShop app and Vampi Api Test Host. This was based off of Stuart Green's work.
...
cpapitrainer # Used to train the WAF API gateway and with -m to create malicious traffic docker-compose down
