...
get_ingress (){
    local svc_json
    svc_json="$(microk8s.kubectl get svc "$WAPAPP" -o json 2>/dev/null) || svc_json='{}'"
    # safe iteration with []? and fallback to empty
    export INGRESS_IP="$(echo "$svc_json" | jq -r '.status.loadBalancer.ingress[]?.ip' 2>/dev/null || true)"
    if [[ -z "${INGRESS_IP:-}" ]]; then
        echo "Warning: Ingress IP not available for service $WAPAPP"
        export INGRESS_IP=""
    fi
}
...
alias cpingress='get_ingress && printf "Ingress IP address used: $INGRESS_IP \n"'
...
alias cpuptemp='get_ingress && echo "Updating coredns.yaml using coredns.yaml.template with local Host IP address of ${INGRESS_IP}" && \
...
cpurltest       Fetches Juiceshop and VAmPI website via Exposed Ingress Controller
